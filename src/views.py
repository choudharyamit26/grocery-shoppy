        ...
        if name and email and password and confirm_password:
            if password != confirm_password:
                ...
                try:
                    ...
                    messages.error(self.request, "Sign up successfull")
                    # return render(request, 'login.html', {"status": 400})
                    return redirect(self.request.path_info)
                except Exception as e:
                    messages.error(self.request, "User with this email already exists. Please supply different email")
                    # return render(request, 'login.html', {"status": 400})
                    return redirect(self.request.path_info)
        else:
            try:
...
class MyOrdersView(ListView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if orders.count() > 0:
            ...
            print('PATH----',self.request.META.get('HTTP_REFERER'))
            if self.request.META.get('HTTP_REFERER') == 'http://127.0.0.1:8000/payment/':
...
class PaymentView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.request.user.is_anonymous:
            messages.success(self.request, 'Please login to make payment')
            return redirect('src:home')
